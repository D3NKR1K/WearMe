...
class SUserAdd(BaseModel):
    ...
    email: EmailStr = Field(..., description="Электронная почта пользователя")
    password: str = Field(..., min_length=8, max_length=32, description="Пароль пользователя, от 8 до 32 символов")
    height: int = Field(None, description="Рост пользователя")
    chest: int = Field(None, description="Мерка обхвата груди пользователя")
    waist: int = Field(None, description="Мерка обхвата талии пользователя")
    hips: int = Field(None, description="Мерка обхвата таза пользователя")

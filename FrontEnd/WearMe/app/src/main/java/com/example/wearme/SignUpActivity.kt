...
class SignUpActivity : AppCompatActivity() {
  ...
  private fun validateEmail(): Boolean {
    ...
    return when {
      ...
      !Patterns.EMAIL_ADDRESS.matcher(email).matches() -> showError(binding.layoutSignUpEmail, getString(R.string.error_invalid_email))
  ...
  private fun validatePassword(): Boolean {
    ...
    return when {
      password.isEmpty() -> showError(binding.layoutSignUpPassword, getString(R.string.error_empty_password))
      password.length < 8 -> showError(binding.layoutSignUpPassword, getString(R.string.error_short_password))
      !password.contains(Regex("[A-Z]")) -> showError(binding.layoutSignUpPassword, getString(R.string.error_no_uppercase))
